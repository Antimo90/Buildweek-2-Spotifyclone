...
const showAlbumDetails = async function (albumTitle) {
  try {
    ...
    albumInfoP.innerHTML = `
      ...
      }' class='me-1 rounded-pill' style='width: 20px'>
      <strong class="text-white artist-link" data-artist-id="${
        albumData.artist.id
      }" style="cursor:pointer;">${albumData.artist.name}</strong> • ${
      albumData.release_date.split("-")[0]
    } • ${albumData.nb_tracks} brani • ${Math.floor(
    ...
    albumInfoP2.innerHTML = `
      ${albumData.release_date.split("-")[0]} <br> ${
      albumData.nb_tracks
    } brani • ${Math.floor(albumData.duration / 60)} min <br>
      <img src='${
        albumData.cover_medium
      }' class='me-1 rounded-pill' style='width: 20px'>
      <strong class="text-white artist-link" data-artist-id="${
    ...
    albumData.tracks.data.forEach((track, index) => {
      tbody.innerHTML += `
        <tr class="text-light">
          ...
          <td class='d-none d-md-table-cell'>${Math.floor(
            track.duration / 60
          )}:${(track.duration % 60).toString().padStart(2, "0")}</td>
...
// Event listener per click su nome artista nella pagina album
if (!window._artistLinkListenerAdded) {
  document.addEventListener("click", function (e) {
    if (e.target.classList.contains("artist-link")) {
      const artistId = e.target.getAttribute("data-artist-id");
      showArtistPage(albumData.artist.id);
    }
  });
  window._artistLinkListenerAdded = true;
}
